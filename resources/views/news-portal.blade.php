        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                @forelse($articles as $article)
                    <article
                        class="border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-200">
                        <a href="/insight/{{ $article->slug }}" class="block p-6">
                            <div class="flex flex-col md:flex-row md:items-center">
                                <div class="md:w-1/4 mb-4 md:mb-0 md:mr-6">
                                    @if ($article->featured_image)
                                        <img class="w-full h-48 md:h-32 object-cover rounded-lg"
                                            src="{{ asset('storage/' . $article->featured_image) }}"
                                            alt="{{ $article->title }}">
                                    @else
                                        <img class="w-full h-48 md:h-32 object-cover rounded-lg"
                                            src="https://via.placeholder.com/800x450" alt="{{ $article->title }}">
                                    @endif
                                </div>
                                <div class="md:w-3/4">
            ...
            <div class="mt-8 text-center">
                <a href="{{ url('/insight') }}"
                    class="inline-flex items-center px-6 py-3 border border-gray-300 text-base font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-300">
