        ...
        <main class="pt-8">
            ...
            <div class="border-b border-gray-200 pb-4 max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center">
                    ...
                    <div class="text-right nyt-section-label">{{ $article->published_at ? $article->published_at->format('d F Y') : $article->created_at->format('d F Y') }}</div>
                </div>
            </div>
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
                ...
                <figure class="mb-10">
                    <div class="aspect-w-16 aspect-h-9 overflow-hidden">
                        <img class="w-full h-auto object-cover" src="{{ asset('storage/' . $article->featured_image) }}" alt="{{ $article->title }}">
                    </div>
                ...
                <div class="text-lg leading-relaxed text-gray-800 mb-12">
                    <div class="nyt-dropcap prose prose-lg max-w-none prose-headings:font-serif prose-headings:font-normal prose-p:text-base prose-p:leading-relaxed">
                        {!! $article->content !!}
                    </div>
                </div>
                            ...
                            <div class="bg-white">
                                <div class="aspect-w-16 aspect-h-9 mb-4">
                                    @if($relatedArticle->featured_image)
                                        <img class="object-cover w-full h-48" src="{{ asset('storage/' . $relatedArticle->featured_image) }}" alt="{{ $relatedArticle->title }}">
                                    @else
                                        <img class="object-cover w-full h-48" src="https://via.placeholder.com/800x450" alt="{{ $relatedArticle->title }}">
                                    @endif
                                </div>
                                <h3 class="text-lg font-semibold text-gray-900 mb-2 leading-tight">{{ $relatedArticle->title }}</h3>
                                <p class="text-sm text-gray-500 mb-2">{{ $relatedArticle->published_at ? $relatedArticle->published_at->format('d F Y') : $relatedArticle->created_at->format('d F Y') }}</p>
                                <a href="/insight/{{ $relatedArticle->slug }}" class="text-gray-900 hover:text-gray-700 text-sm font-medium nyt-section-label">BACA ARTIKEL →</a>
                            </div>
                        @endforeach
                    </div>
                </div>
